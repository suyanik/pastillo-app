...
import { TrendingUp, TrendingDown, Wallet, CreditCard, ArrowRight, Receipt, Plus } from 'lucide-react';
...
const Dashboard: React.FC<Props> = ({ records, lang }) => {
  ...
  return (
    <div className="space-y-6">
      {/* Main Stats Card */}
      <div className="bg-gradient-to-br from-primary via-primary/80 to-primary/40 rounded-[2.5rem] p-8 relative overflow-hidden shadow-[0_20px_40px_rgba(255,193,7,0.15)] ring-1 ring-white/10">
        <div className="absolute -right-4 -top-4 w-40 h-40 bg-white/20 rounded-full blur-[60px]"></div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="glass p-5 rounded-[2rem] border border-white/5 relative group">
          <div className="p-3 bg-orange-500/20 rounded-2xl w-fit mb-4 text-orange-500 group-hover:scale-110 transition-transform">
            <Wallet size={24} />
          </div>
        ...
        <div className="glass p-5 rounded-[2rem] border border-white/5 relative group">
          <div className="p-3 bg-blue-500/20 rounded-2xl w-fit mb-4 text-blue-500 group-hover:scale-110 transition-transform">
            <CreditCard size={24} />
          </div>
      ...
      <div className="space-y-4 pt-2">
        <div className="flex justify-between items-center px-1">
          ...
          <button className="text-primary text-[10px] font-black flex items-center gap-1 hover:gap-2 transition-all">
        ...
        <div className="space-y-3">
           {[
             ...
             { title: 'Elektrik Faturası', cat: 'Kira/Fatura', price: 450, time: 'Dün' },
           ].map((item, i) => (
             <div key={i} className="glass p-5 rounded-[1.8rem] flex items-center justify-between border border-white/5 group hover:bg-white/10 transition-all cursor-pointer">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center text-white/20 group-hover:text-primary group-hover:bg-primary/10 transition-all">
                    <Receipt size={22} />
                  </div>
                  <div>
